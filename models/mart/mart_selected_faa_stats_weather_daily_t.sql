...
--			,COUNT(DISTINCT airline) AS dep_nunique_airlines -- BONUS TASK
	FROM {{ref('prep_flights_t')}}
	WHERE origin IN (SELECT DISTINCT airport_code FROM {{ref('prep_weather_daily_t')}})
...
--			,COUNT(DISTINCT airline) AS arr_nunique_airlines -- BONUS TASK
	FROM {{ref('prep_flights_t')}}
	WHERE dest IN (SELECT DISTINCT airport_code FROM {{ref('prep_weather_daily_t')}})
...
SELECT t.* 
		...
		,w.wind_peakgust_kmh
FROM total_stats t
LEFT JOIN {{ref('prep_weather_daily_t')}} w
